...
module Main where
    ...
    placeAllShips board n =
        do
            ...
            if((dir::Int) == 0)
                then
                    if(isShipPlaceableY n x y n board)
                    then
                        if(n==1)then return ((placeShipy 2 (x::Int) (y::Int) 1 board)::Board)
                            else return ((placeShipy n (x::Int) (y::Int) n board)::Board)
                    else placeAllShips board n
            else
                if(isShipPlaceableX n x y n board)
                    then
                        if(n==1)then return ((placeShipx 2 (x::Int) (y::Int) 1 board)::Board)
                            else return ((placeShipx n (x::Int) (y::Int) n board)::Board)
                    else placeAllShips board n

    main =
        do
            board <- placeAllShips board 5
            board <- placeAllShips board 4
            board <- placeAllShips board 3
